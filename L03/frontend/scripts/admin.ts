...
document.addEventListener("DOMContentLoaded", async() =>{
    ...
    const updateForm = document.getElementById("updateImageForm") as HTMLFormElement;
...
async function update(event: Event){
    ...
    let checkboxes:NodeListOf<HTMLInputElement> = document.querySelectorAll("#updateImageForm input[type=\"checkbox\"]");
    let data = new URLSearchParams();
    let result : {source: string, enabled: boolean}[] = [];
