...
export namespace A09Server {
  ...
  async function getImages(_response: Http.ServerResponse, _query: ParsedUrlQuery): Promise<void> {
  ...
  async function getAllowedImages(_response: Http.ServerResponse, _query: ParsedUrlQuery): Promise<void> {
    _response.setHeader("content-type", "application/json");
    let images = await client.db("memory").collection("images").find({enabled: true}).toArray();
    _response.write(JSON.stringify({success: true, images: images}));
  }

  async function getScores(_response: Http.ServerResponse, _query: ParsedUrlQuery): Promise<void> {
    _response.setHeader("content-type", "application/json");
  ...
  async function submitScore(_response: Http.ServerResponse, _query: ParsedUrlQuery): Promise<void> {
    _response.setHeader("content-type", "application/json");
  ...
  async function updateImages(_response: Http.ServerResponse, _query: ParsedUrlQuery): Promise<void> {
    _response.setHeader("content-type", "application/json");
    if(!_query.data){
      _response.write(JSON.stringify({success: false}));
    }
    else{
      let updatedData = JSON.parse(_query.data as string);
